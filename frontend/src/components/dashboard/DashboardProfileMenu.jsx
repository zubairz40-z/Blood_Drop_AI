    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <div className="hidden sm:flex flex-col items-start">
          ...
          <span className="text-[11px] text-white/70 font-medium leading-tight">{role}</span>
        </div>
        <ChevronDown className={`w-4 h-4 text-white/60 transition-transform duration-200 hidden sm:block ${open ? 'rotate-180' : ''}`} />
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-border rounded-xl shadow-elevated overflow-hidden z-50">
          <div className="px-4 py-3 border-b border-border bg-brand-soft/20">
            <p className="text-sm font-semibold text-text-dark">{name}</p>
            <Badge variant={roleBadgeVariant[role] || 'primary'} className="mt-1">{role}</Badge>
          </div>
          <div className="py-1">
            <Link
              to="#"
              className="flex items-center gap-3 px-4 py-2.5 text-sm text-text-secondary hover:bg-brand-soft/30 hover:text-brand transition-colors"
